...
class Api:
    ...
    def get_init_state(self):
        ...
        return {
            'view': 'dashboard' if (has_creds and len(self.courses) > 0) else 'login',  # 🔑 修复：需要凭证且有课程数据
            'config': config,
            'state': state,
            'courses': self.courses,  # 🔑 返回课程列表
            'local_stats': local_stats,
            'should_auto_sync': has_creds and config.get('auto_sync', False) and len(self.courses) > 0  # 🔑 标记是否需要自动同步
        }
        ...
        try:
            if self._config_path.exists():
                ...
                return {
                    ...
                    'auto_sync': cfg.getbool('auto_sync', False),  # 🔑 添加auto_sync配置
    ...
    def save_config(self, data):
        ...
        try:
            ...
            parser.set('Advanced', 'auto_sync', str(data.get('auto_sync', False)).lower())  # 🔑 保存auto_sync设置
            parser.set('Advanced', 'language', data.get('language', 'en'))  # 🔑 保存语言设置
    ...
    def _save_all_courses(self):
        ...
        try:
            ...
            for course in self.courses:
                ...
                entry['available_tabs'] = course.get('available_tabs', [])  # 🔑 保存标签页列表
